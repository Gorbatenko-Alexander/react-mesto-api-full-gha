    ...
    return (
      <div className="places__place-card">
        ...
        <div className="places__place-like-container">
          <button type="button"
                  className={`places__place-like ${this.props.card.likes.some(like => like._id === this.context._id) && 'places__place-like_active'}`}
                  onClick={() => {this.handleLikeClick()}}></button>
          <p className="places__place-like-number">{this.props.card.likes.length}</p>
        </div>
        <button type="button"
                className={`places__place-remove ${!(this.props.card.owner._id === this.context._id) && 'places__place-remove_disabled'}`}
